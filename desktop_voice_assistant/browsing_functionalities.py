...
def get_speedtest():
    try:
        ...
        download_speed = round(internet.download() / 1_000_000, 2)  # in Mbps
        upload_speed = round(internet.upload() / 1_000_000, 2)  # in Mbps
        speed = f"Your network's Download Speed is {download_speed} Mbps\n" \
                f"Your network's Upload Speed is {upload_speed} Mbps"
        return speed
    except (speedtest.SpeedtestException, KeyboardInterrupt) as e:
        return "Speed test failed or was interrupted."
